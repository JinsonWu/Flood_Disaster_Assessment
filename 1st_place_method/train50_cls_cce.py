...
ourweight_folder = './flood_weights/'
...
class TrainData(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        img2 = cv2.imread(fn.replace('/pre_img/', '/post_img/').replace('_pre_', '_post_'), cv2.IMREAD_COLOR)

        msk0 = cv2.imread(fn.replace('/pre_img/', '/pre_msk/').replace('_pre_', '_post_'), cv2.IMREAD_UNCHANGED)
        lbl_msk1 = cv2.imread(fn.replace('/pre_img/', '/post_msk/').replace('_pre_', '_post_'), cv2.IMREAD_UNCHANGED)
        msk0 = msk0[..., 0]
        lbl_msk1 = lbl_msk1[..., 0]
...
def evaluate_val(data_val, best_score, model, snapshot_name, current_epoch):
    ...
    torch.save({
        ...
        'F1_1': d[1],
        'F1_2': d[2],
    }, path.join(ourweight_folder+'/50_2_2/', snapshot_name + '_epoch' + str(current_epoch+1)))
    best_score = f1
...
if __name__ == '__main__':
    ...
    for fn in tqdm(all_files):
        ...
        msk1 = cv2.imread(fn.replace('/pre_img/', '/post_msk/').replace('_pre_', '_post_'), cv2.IMREAD_UNCHANGED)
        msk1 = msk1[..., 0]
        for c in range(1, 4):
            fl[c-1] = c in msk1
    ...
    for epoch in range(200):
        ...
        best_li.append(best_score)

    plt.plot(np.arange(200), np.asarray(best_li))
    plt.savefig(ourweight_folder + '/50_2_2/F1_Score.png')
