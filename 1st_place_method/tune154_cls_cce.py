...
class TrainData(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        lbl_msk1 = cv2.imread(fn.replace('/image/', '/mask/'), cv2.IMREAD_UNCHANGED)
        lbl_msk1 = lbl_msk1[..., 0]
...
def evaluate_val(data_val, best_score, model, snapshot_name, current_epoch):
    model = model.eval()
    f1_sc, d = validate(model, data_loader=data_val)
...
def train_epoch(current_epoch, seg_loss, ce_loss, model, optimizer, scheduler, train_data_loader):
    ...
    for i, sample in enumerate(iterator):
        ...
        loss = 0.5 * loss0 + 0.2 * loss1 + 0.2 * loss2 + 0.05 * loss3 + loss5 * 1
...
if __name__ == '__main__':
    ...
    snap_to_load = 'se154_cls_cce_{}_best'.format(seed)
