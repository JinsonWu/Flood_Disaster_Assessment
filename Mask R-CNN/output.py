...
cfg.MODEL.WEIGHTS = os.path.join(cfg.OUTPUT_DIR, "model_0049999.pth")  # path to the model we just trained
cfg.MODEL.ROI_HEADS.SCORE_THRESH_TEST = 0.5   # set a custom testing threshold
#cfg.INPUT.COLOR_AUG_SSD = False


predictor = DefaultPredictor(cfg)
dataset_dicts = glob.glob('./post_img/')
...
for d in dataset_dicts: # shuffle the validation dataset
    ...
    np.savetxt(d.replace('.png', '.csv'), out_mask, delimiter=',')
