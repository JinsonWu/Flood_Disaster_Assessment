...
def manual_classes():
    ...
    manual_dict = {
        'Background': 0,
        'Flood/Water': 0,
        'Non-Flooded Building': 255,
        'Flooded Building': 255,
    }
    return manual_dict
...
for image in json_file:
    ...
    # read original image
    original_img = open_img(image['Labeled Data'])
    ...
    for i in range(len(image['Label']['objects'])):
        # read path and open image
        img = open_img(image['Label']['objects'][i]['instanceURI'])

        # if path is not readable try to read polygon-data-points
        if not img is None:
            img = img_color(img, color_extractor(image['Label']['objects'][i], color_type))
            img_list.append(img)
        else:
            try:
                # img = img_draw_polygon(img, data[0]['Label']['objects'][i]['polygon'], data[0]['Label']['objects'][i]['title'])
                img = img_draw_polygon((width,height), image['Label']['objects'][i]['polygon'], color_extractor(image['Label']['objects'][i], color_type))
        ...
        progressBar(i, len(image['Label']['objects']))

        img = img_list[0]
        for i in range(1, len(img_list)):
            img.paste(img_list[i], (0,0), mask= img_list[i])
        ptr1 = image['Labeled Data'].find('Area')
        ptr2 = image['Labeled Data'].find('.png') + 4
        img.save(output_pth + image['Labeled Data'][ptr1:ptr2])